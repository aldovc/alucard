...
- `<ticket>` — what the worker was working on, when the harness knows. For a GitHub issue, `gh issue view <N> --comments` gives the acceptance criteria.
- `<stop_reason>` — how the worker stopped.
- `<base_branch>` — the branch this work would merge into. `origin/<base_branch>` exists in this checkout: `git log origin/<base_branch>..HEAD --stat` is what the worker committed, `git diff` what it did not.
...
1. **Look.** `git status`, `git diff --stat`, `git log origin/<base_branch>..HEAD --stat`. Read the ticket's acceptance criteria if there is a ticket. Open the files the diff touches only as far as you need to name what they do.
