...
Two review loops on this repository each spent five cycles on one class of defect, because every cycle found a single site and stopped there. Every finding in both was correct and worth making. They simply arrived one at a time, and each one cost a full review, a feedback agent, and a CI run.

Generalise before you search. "This retry has no upper bound" is a site; "a wait continues forever when the thing it waits for never arrives" is the class. "This cache key leaves out the tenant" is a site; "an identifier is not unique across every axis its data varies on" is the class. Once you can name the class, `rg` for its other sites across the diff and the files the diff touches.
...
- **Abstraction bypass**: if the codebase already has a helper for an external call (a send/notify wrapper, an API client), new code touching the same API must use it — never re-implement the same call inline.
- **Partial abstraction**: when the diff introduces a class/module to consolidate a scattered pattern but migrates only some callsites (e.g. one method moved to a new client while its siblings keep their inline HTTP calls), flag it. A half-migrated abstraction leaves timeout and error logic diverging across multiple places — worse than not extracting at all. Either every callsite of the old pattern is migrated or the PR body names the ones deliberately deferred.
- **Interface change not propagated**: when a Protocol or shared method signature gains a parameter, list every implementation and caller and flag any that the diff left unchanged. A new optional kwarg wired into one entry point but not its sibling entry points silently makes the feature unreachable from the others.
