...
assert_eq "empty queue emits three iteration/run transitions" "3" "${#empty_events[@]}"
assert_eq "empty queue starts the iteration first" \
  "Iteration 1/1 start" "${empty_events[0]:-}"
assert_matches "empty queue closes the iteration with rc=0" \
  '^Iteration 1/1 end: rc=0 duration=[0-9]+s$' "${empty_events[1]:-}"
assert_eq "empty queue records the run end last" \
